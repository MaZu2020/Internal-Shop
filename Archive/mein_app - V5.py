...
image_width = "50px"     # Breite des Bildes einstellen
image_height = "50px"    # Höhe des Bildes einstellen
...
# Funktion zum Speichern einer Bestellung in der Excel-Datei
def save_order(store_number, sap_number, quantity):
    # Bestellungen aus Excel laden
    orders_df = pd.read_excel(orders_path, index_col=0)

    # Prüfen, ob die Storenummer und die SAP-Nummer existieren
    ...
    st.success(f"{_("order_success")}")

# Funktion zum Versenden einer E-Mail mit mailto-Link
...
"""
    ...
    mailto_link = f"mailto:{email_recipient}?subject={email_subject}&body={email_body.replace(chr(10), '%0A')}"
